...
const HistoryLog = ({ logData, idRequest }) => {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorFn: (row) => formatColumnName(row.column),
        id: "column",
        header: "Column",
        enableSorting: true,
      ...
      },
      {
        ...
        cell: ({ row }) => renderAuditValue(row.original.before),
      },
      {
        accessorKey: "after",
        id: "after",
        ...
        enableSorting: true,
        enableColumnFilter: true,
        cell: ({ row }) => renderAuditValue(row.original.after),
      },
      {
  ...
  const getLogData = useCallback(async () => {
    ...
    try {
      ...
      const flattened = flattenAuditLogs(res.data.data);
