...
export default function ProjectList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(`${API_URL}/portal-project`, {
  ...
  const handleDelete = async (id) => {
    const result = await showAlert("Delete Project", "question", "Are you sure?", "Yes, Delete", true);
      ...
      cell: ({ row }) => {
        ...
        return (
          <Group gap={6} justify="center" wrap="nowrap">
            <Button size="xs" color="blue" leftSection={<IconEdit size={14} />}
              onClick={() => router.push(`/user_management/project/edit/${project.id_project}`)}>
              Edit
            </Button>
  ...
  return (
    <>
      <Head><title>Project Management | ARMC</title></Head>
      <AuthLayout sidebarList={userList}>
        <div className="py-6 px-4">
          <Paper radius="md" p="md" withBorder shadow="sm">
            <div className="flex justify-between border-b pb-4 mb-4">
              <div>
                <h1 className="text-md font-extrabold text-teal-600 uppercase">Project Management</h1>
                <p className="text-xs text-gray-500">Manage projects</p>
              </div>
              <Button size="sm" color="teal" leftSection={<IconPlus size={16} />}
                onClick={() => router.push(`/user_management/project/add`)}>
                Add Project
              </Button>
            </div>
